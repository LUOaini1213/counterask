import { boot, harness, toolNames as _tn } from "./lib/boot.mjs";
const window = boot({ url: "https://example.test/" });
const $ = (s) => window.document.querySelector(s);
const toolNames = () => _tn(window);

const { ck: check, done } = harness();
...
// empty the catalog in a way that holds on any catalog: a narrow pool, then a
// requirement nothing in it records — pick the facet value least present
await mc.callTool("search_products", { query: "a watch" });
const voc = window.Engine.attributeVocabulary();
const rarest = voc.material.slice().sort((a, b) => a.count - b.count)[0].value;
let dead = await mc.callTool("refine_search", { require: rarest });
if (dead.structuredContent.candidates > 0) {
  const rarest2 = (voc.closure || voc.material).slice().sort((a, b) => a.count - b.count)[0].value;
  dead = await mc.callTool("refine_search", { require: rarest2 });
}
check("zero candidates", dead.structuredContent.candidates === 0, dead.structuredContent.candidates + " left");
