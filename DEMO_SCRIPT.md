> **Note:** the on-screen numbers below were read off the *synthetic* build
> (`counterask-demo-lite.html`). On the real catalog the same beats hold — `belt`
> asks about material, `leather belt` answers, `shoes` asks *what kind* and
> walks the tree — but the counts differ. Re-read them off whichever build you
> record before you say them.
